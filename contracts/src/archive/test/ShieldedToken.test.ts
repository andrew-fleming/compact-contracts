...
describe('Shielded token', () => {
  ...
  describe('mint', () => {
    it('should mint', () => {
      ...
      const thisNonce = token.getPublicState().ShieldedToken__nonce;
      const thisCoinInfo = {
    ...
    it('should bump counter', () => {
      ...
      expect(token.getPublicState().ShieldedToken__counter).toEqual(1n);
    });

    it('should bump nonce', () => {
      const initNonce = token.getPublicState().ShieldedToken__nonce;
      ...
      expect(initNonce).not.toEqual(
        token.getPublicState().ShieldedToken__nonce,
      );
    });
